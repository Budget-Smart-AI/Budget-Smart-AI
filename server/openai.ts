...
// Helper to choose between OpenAI and DeepSeek based on configuration
export function getAIClient(useDeepSeek: boolean = true) {
  return useDeepSeek ? deepseek : openai;
}

export function getModelForTask(complexity: "simple" | "moderate" | "complex" = "moderate", useDeepSeek: boolean = true): string {
  if (useDeepSeek) {
    switch (complexity) {
      case "simple":
    ...
    return "gpt-4o"; // Fallback to OpenAI
  }
}


// Define all tools the AI can use to access financial data
...
export const SYSTEM_PROMPT = `You are Budget Smart AI, a personal finance expert assistant. You are integrated into the Budget Smart AI budgeting application. You have access to the user's real financial data including their bills, expenses, income, bank accounts, bank transactions, budgets, and savings goals.
