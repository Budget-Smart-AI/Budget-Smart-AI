# BudgetSmart Policy Documentation — Index

**Company:** BudgetSmart  
**Owner:** Ryan Mahabir, CEO  
**Email:** ryan@mahabir.pro  
**Version:** 1.0  
**Last Updated:** March 9, 2026  

> **SOC 2 Status:** Formal SOC 2 Type I audit begins June/July 2026 via Comp AI. Expected certified: August 2026.

---

## Overview

This directory contains BudgetSmart's formal policy documentation supporting SOC 2 Type I certification and ongoing compliance with PIPEDA (Canada) and applicable U.S. privacy regulations.

All policies are owned by Ryan Mahabir (CEO) unless otherwise noted and are reviewed on the schedule listed below.

---

## Document Index

| # | Document | Version | Effective Date | Owner | Review Schedule |
|---|----------|---------|---------------|-------|----------------|
| 1 | [Information Security Policy](./information-security-policy.md) | 1.1 | March 7, 2026 | Ryan Mahabir | Annual |
| 2 | [Access Control Policy](./access-control-policy.md) | 1.1 | March 7, 2026 | Ryan Mahabir | Annual |
| 3 | [Incident Response Plan](./incident-response-plan.md) | 1.0 | March 7, 2026 | Ryan Mahabir | Annual / After P1 or P2 incident |
| 4 | [Data Retention Policy](./data-retention-policy.md) | 1.0 | March 7, 2026 | Ryan Mahabir | Annual |
| 5 | [Vendor Management Policy](./vendor-management-policy.md) | 1.1 | March 7, 2026 | Ryan Mahabir | Annual |
| 6 | [Change Management Policy](./change-management-policy.md) | 1.1 | March 7, 2026 | Ryan Mahabir | Annual |
| 7 | [Business Continuity Plan](./business-continuity-plan.md) | 1.1 | March 7, 2026 | Ryan Mahabir | Annual / After DR event |
| 8 | [Privacy Policy](./privacy-policy.md) *(public-facing)* | 1.0 | March 7, 2026 | Ryan Mahabir | Annual |
| 9 | [Terms of Service](./terms-of-service.md) *(public-facing)* | 1.1 | March 9, 2026 | Ryan Mahabir | Annual |
| 10 | [Risk Register](./risk-register.md) | 1.0 | March 7, 2026 | Ryan Mahabir | Quarterly |
| 11 | [SOC 2 Controls Matrix](./soc2-controls-matrix.md) | 1.1 | March 7, 2026 | Ryan Mahabir | Annual / Pre-audit |

---

## SOC 2 Certification Timeline

| Milestone | Target Date |
|-----------|------------|
| Policy documentation complete | March 2026 ✅ |
| Security controls hardening complete (MFA, branch protection, Dependabot, Snyk, secret scanning, push protection, CodeQL, NeonDB IP restriction, read-only DB role, field encryption, uptime monitoring) | March 2026 ✅ |
| Comp AI onboarding and gap assessment | March–April 2026 |
| Controls implementation and evidence collection | April–June 2026 |
| SOC 2 Type I audit begins | June/July 2026 |
| Expected SOC 2 Type I certification | August 2026 |
| SOC 2 Type II audit window begins | ~September 2026 |

---

## Policy Coverage Summary

| SOC 2 Area | Covered By |
|-----------|-----------|
| Security (CC6) | Information Security Policy, Access Control Policy |
| Availability (A1) | Business Continuity Plan |
| Confidentiality | Information Security Policy, Data Retention Policy |
| Privacy | Privacy Policy, Data Retention Policy |
| Risk Management (CC3, CC9) | Risk Register, Vendor Management Policy |
| Change Management (CC8) | Change Management Policy |
| Incident Response (CC7) | Incident Response Plan |
| Control Environment (CC1–CC5) | SOC 2 Controls Matrix |

---

## Public-Facing Documents

The following documents are published on the BudgetSmart website:

| Document | URL |
|----------|-----|
| Privacy Policy | budgetsmart.io/privacy |
| Terms of Service | budgetsmart.io/terms |

---

## Document Control

All policy documents are maintained in the BudgetSmart GitHub repository under `/docs/policies/`. Changes follow the [Change Management Policy](./change-management-policy.md) — all updates must be submitted via GitHub Pull Request and approved by Ryan Mahabir.

Prior versions are preserved in Git history.

---

## Contact

For questions about these policies:  
**Ryan Mahabir** — ryan@mahabir.pro — budgetsmart.io  
Hamilton, Ontario, Canada

---

*BudgetSmart — Hamilton, Ontario, Canada | budgetsmart.io*
